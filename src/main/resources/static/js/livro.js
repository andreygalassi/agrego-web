const app = Vue.createApp({
	...
	data() {
		return {
			...
			headers: [
				...
				{ title: 'Titulo', key: 'titulo', align: 'left', sortable: true, },
				{ title: 'Descrição', key: 'descricao', align: 'left', sortable: true, },
				{ title: 'Autor', key: 'autor.nome', align: 'right', sortable: true, },
				{ title: 'Ação', key: 'actions', align: 'end', sortable: false, width:5 },
			],
			api: {
				principal: "../api/livro",
				listaAutor: "../api/autor",
			},
			consulta: {
				filtro: {},
				resultado: [],
	...
	methods: {
		carregaDominio() { 
			this.$axios.get(this.api.listaAutor, {params: {}, headers:{}}).then(
				(response) => {
					this.listasInternas.listaAutor = response.data.content;
				}
			).catch(
				(error) => {
